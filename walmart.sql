...
select branch,round(sum(unit_price*quantity),2) as revenue from walmart
group by branch order by revenue limit 10;

select city,count(distinct branch) as total_branch from walmart
group by city order by total_branch desc;
...
from walmart group by Branch order by total_profit desc;
...
from walmart group by  Branch order by avg_rating desc;
...
count(*) as count from walmart group by price_range order by count desc;
...
select Branch, category, sum(unit_price * quantity) as revenue, sum(quantity) as uantity
...
select  branch, sum(unit_price * quantity) AS cash_revenue
...
select invoice_id,branch,city,unit_price,sum(unit_price) over(order by invoice_id) as rn from walmart;
